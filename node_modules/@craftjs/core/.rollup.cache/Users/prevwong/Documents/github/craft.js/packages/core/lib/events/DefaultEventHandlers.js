...
export class DefaultEventHandlers extends CoreEventHandlers {
        ...
        return {
            ...
            drag: (el, id) => {
                ...
                const unbindDragStart = this.addCraftEventListener(el, 'dragstart', (e) => {
                    ...
                    DefaultEventHandlers.draggedElementShadow = createShadow(e, query.node(id).get().dom, selectedDOMs);
                    DefaultEventHandlers.draggedElement = selectedElementIds;
                });
                const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
                    e.craft.stopPropagation();
                    const onDropElement = (draggedElement, placement) => {
                        const index = placement.index + (placement.where === 'after' ? 1 : 0);
    ...
    dropElement(onDropNode) {
        const store = this.options.store;
        const { draggedElement, draggedElementShadow, indicator, } = DefaultEventHandlers;
