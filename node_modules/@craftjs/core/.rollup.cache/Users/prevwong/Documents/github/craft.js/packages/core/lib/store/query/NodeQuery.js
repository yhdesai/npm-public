import { deprecationWarning, ERROR_CANNOT_DRAG, ERROR_MOVE_CANNOT_DROP, ERROR_MOVE_INCOMING_PARENT, ERROR_MOVE_NONCANVAS_CHILD, ERROR_MOVE_OUTGOING_PARENT, ERROR_MOVE_TOP_LEVEL_NODE, ERROR_MOVE_TO_DESCENDANT, ERROR_MOVE_TO_NONCANVAS_PARENT, ERROR_NOT_IN_RESOLVER, ROOT_NODE, } from '@craftjs/utils';
...
export class NodeQuery {
     ...
     */
    _createBackwardsCompatibleNode(node) {
    ...
    getAncestors() {
        const appendParentNode = (id, ancestors = [], depth = 0) => {
    ...
    getDescendants() {
        const appendChildNode = (id, descendants = [], depth = 0) => {
            const node = this.find(id);
            if (!node) {
                return descendants;
            }
    ...
    getLinkedNodes() {
        return Object.entries(this.node.linkedNodes).reduce((accum, [linkedId, nodeId]) => [
            ...accum,
    ...
    isDraggable(onError) {
        try {
            ...
            invariant(this.find(this.node.parent).isCanvas(), ERROR_MOVE_NONCANVAS_CHILD);
            invariant(this.getRules().canDrag(this._createBackwardsCompatibleNode(this.node), (id) => this.find(id)), ERROR_CANNOT_DRAG);
            return true;
        }
        catch (err) {
            if (onError) {
    ...
    isDroppable(selector, onError) {
        ...
        try {
            ...
            invariant(this.getRules().canMoveIn(targets.map((selector) => this._createBackwardsCompatibleNode(selector.node)), this._createBackwardsCompatibleNode(this.node), (id) => this.find(id)), ERROR_MOVE_INCOMING_PARENT);
            const parentNodes = {};
            targets.forEach(({ node: targetNode, exists }) => {
                const rules = this.getRules();
                invariant(rules.canDrop(this._createBackwardsCompatibleNode(this.node), this._createBackwardsCompatibleNode(targetNode), (id) => this.find(id)), ERROR_MOVE_CANNOT_DROP);
                // Ignore other checking if the Node is new
                if (!exists) {
                    return;
                }
                const targetDeepNodes = this.descendants();
                invariant(!targetDeepNodes.includes(this.id) && this.id !== targetNode.id, ERROR_MOVE_TO_DESCENDANT);
            ...
            Object.keys(parentNodes).forEach((parentNodeId) => {
                ...
                invariant(parentNode.getRules().canMoveOut(childNodes.map((node) => this._createBackwardsCompatibleNode(node)), this._createBackwardsCompatibleNode(parentNode.node), (id) => this.find(id)), ERROR_MOVE_OUTGOING_PARENT);
            });
            return true;
        }
        catch (err) {
